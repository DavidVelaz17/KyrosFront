...
import { PAGO_COLUMN_CATALOG } from "@/lib/constants/pago-columns";

interface PagosColumnVisibilityMenuProps {
  visibility: Record<string, boolean>;
  onChange: (visibility: Record<string, boolean>) => void;
}

export function PagosColumnVisibilityMenu({ visibility, onChange }: PagosColumnVisibilityMenuProps) {
  ...
  return (
    ...
    >
      <div className="max-h-80 overflow-y-auto px-1 py-1">
        ...
        {PAGO_COLUMN_CATALOG.map((column) => (
          <label
            key={column.id}
