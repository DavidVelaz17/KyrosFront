...
import { CARGO_COLUMN_CATALOG } from "@/lib/constants/cargo-columns";

interface CargosColumnVisibilityMenuProps {
  visibility: Record<string, boolean>;
  onChange: (visibility: Record<string, boolean>) => void;
}

export function CargosColumnVisibilityMenu({ visibility, onChange }: CargosColumnVisibilityMenuProps) {
  ...
  return (
    ...
    >
      <div className="max-h-80 overflow-y-auto px-1 py-1">
        ...
        {CARGO_COLUMN_CATALOG.map((column) => (
          <label
            key={column.id}
